...
(function($) {
	...
	function _14(_15) {
		...
		if ($(_15).find(".calendar-menu-month-inner").is(":empty")) {
			...
			var t = $("<table></table>").appendTo(
					$(_15).find(".calendar-menu-month-inner"));
			var idx = 0;
			for (var i = 0; i < 3; i++) {
				var tr = $("<tr></tr>").appendTo(t);
				for (var j = 0; j < 4; j++) {
					$("<td class=\"calendar-menu-month\"></td>").html(
							_16.months[idx++]).attr("abbr", idx).appendTo(tr);
				}
			}
			$(_15).find(".calendar-menu-prev,.calendar-menu-next").hover(
					function() {
						$(this).addClass("calendar-menu-hover");
					}, function() {
						$(this).removeClass("calendar-menu-hover");
			...
			$(_15).find(".calendar-menu-month").hover(function() {
				...
				$(this).removeClass("calendar-menu-hover");
					...
					function() {
						...
						_18.find(".calendar-selected").removeClass(
								"calendar-selected");
	...
	function _20(_21, _22, _23) {
		...
		if (_2b.length < 7) {
			while (_2b.length < 7) {
				...
				_2b.unshift([ _2a.getFullYear(), _2a.getMonth() + 1,
						_2a.getDate() ]);
			}
		} else {
			var _2c = _2b[0];
			...
			for (var i = 1; i <= 7; i++) {
				...
				_28.unshift([ _2a.getFullYear(), _2a.getMonth() + 1,
						_2a.getDate() ]);
			}
			_27.unshift(_28);
		}
		...
		if (_27.length < 6) {
			...
			for (var i = 1; i <= 7; i++) {
				...
				_28
						.push([ _2a.getFullYear(), _2a.getMonth() + 1,
								_2a.getDate() ]);
			}
			_27.push(_28);
		}
	...
	function _d(_2f) {
		...
		$(_2f).find(".calendar-title span").html(
				_30.months[_30.month - 1] + " " + _30.year);
		...
		t.find("td[abbr^=\"" + _30.year + "," + _30.month + "\"]").removeClass(
				"calendar-other-month");
		var now = new Date();
		var _34 = now.getFullYear() + "," + (now.getMonth() + 1) + ","
				+ now.getDate();
		t.find("td[abbr=\"" + _34 + "\"]").addClass("calendar-today");
		if (_30.current) {
			t.find(".calendar-selected").removeClass("calendar-selected");
			var _35 = _30.current.getFullYear() + ","
					+ (_30.current.getMonth() + 1) + ","
					+ _30.current.getDate();
	...
	$.fn.calendar = function(_39, _3a) {
		...
		return this.each(function() {
			...
			if (_3b) {
				...
				_3b = $.data(this, "calendar", {
					options : $.extend({}, $.fn.calendar.defaults,
							$.fn.calendar.parseOptions(this), _39)
				});
				_5(this);
			}
	...
	$.fn.calendar.methods = {
		...
		moveTo : function(jq, _3c) {
			return jq.each(function() {
				$(this).calendar({
					year : _3c.getFullYear(),
					month : _3c.getMonth() + 1,
					current : _3c
				});
			});
		}
